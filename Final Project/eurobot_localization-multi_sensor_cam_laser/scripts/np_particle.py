...
class ParticleFilter:
    ...
    def motion_model(self, delta):
        ...
        if delta_imu is not None:
            cov_theta = 0.005 + 0.06 * delta_imu[2] ** 2
        ...
        except (RuntimeError, ValueError, np.VisibleDeprecationWarning) as e:
            pass
    ...
    def weights(self, landmarks, particles, global_coords):
        ...
        if landmarks.shape[0] != 0:
            ...
            weights = self.gaus(r_diff, mu=0, sigma=self.sigma_r) * self.gaus(phi_diff, mu=0, sigma=self.sigma_phi)
