...
terms_main = [
    1.9312803901331665e-002,
    1.0046955169996832e+000,
    -9.8182321164347663e-002,
    2.1855775901441124e-001,
    -2.2112847955574078e-001,
    1.2256116999654873e-001,
    -3.8132712090098907e-002,
    6.2192698197484353e-003,
    -4.1260482875595244e-004
]
...
class PFNode(object):
    ...
    def scan_callback(self, data):
        ...
        if np.linalg.norm(delta_lidar[:2]) >= 0.2 or abs(delta_lidar[2]) >= 0.6 or scan_time.to_sec() - \
                lidar_odom_coords[0] >= 0.5:
            # rospy.logwarn("Lidar odom error %s" % delta_lidar)
            delta_lidar = None

        if np.linalg.norm(delta_odom[:2]) >= 0.2 or abs(delta_odom[2]) >= 0.6 or scan_time.to_sec() - wheel_odom_coords[
            0] >= 0.5:
        ...
        # # rospy.logwarn(cvt_local2global(beacons, self.robot_filtered_coords))
        # if len(beacons) > 0:
        #     self.beacon_ranges.append(np.linalg.norm(beacons[0]))
        #     if len(self.beacon_ranges) >= 100:
        #         ranges = np.array(self.beacon_ranges)
        #         rospy.logwarn(np.mean(ranges))
        #         self.beacon_ranges = []
    ...
    def point_cloud_from_scan(self, scan):
        ...
        ranges = [regress(r) if r <= 3 else r + self.scan_offset for r in ranges]
